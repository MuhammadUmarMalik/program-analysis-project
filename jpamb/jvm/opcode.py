...
@dataclass(frozen=True, order=True)
class Pop(Opcode):
    """The pop opcode that removes the top value from the operand stack."""
...
class InvokeVirtual(Opcode):
    ...
    def real(self):
        return f"invokevirtual {str(self.method)}"
...
class InvokeDynamic(Opcode):
    """The invokedynamic opcode (dynamic call site / bootstrap)."""

    method: jvm.AbsMethodID
    index: int  # index into the bytecode's dynamic pool (if present)

    @classmethod
    def from_json(cls, json: dict) -> "Opcode":
        # The decompiler emits a 'method' entry describing args/name/returns
        # but not a class reference. Create a synthetic AbsMethodID with a
        # placeholder classname so other code can inspect name/params/returns.
        meth = json.get("method", {})
        name = meth.get("name")
        args = meth.get("args", [])
        returns = meth.get("returns")

        params = jvm.ParameterType.from_json(args)
        ret_type = jvm.Type.from_json(returns) if returns is not None else None

        absm = jvm.AbsMethodID(
            classname=jvm.ClassName.decode("<dynamic>"),
            extension=jvm.MethodID(name=name, params=params, return_type=ret_type),
        )

        return cls(offset=json["offset"], method=absm, index=json.get("index", 0))
    ...
    def __str__(self):
        return f"invoke dynamic {self.method} (index={self.index})"
