...
def _clean_atom(s: str) -> str:
    t = str(s).strip()

    # unwrap single parens e.g. "(int 0)" or "(0)" -> "int 0" / "0"
    if t.startswith("(") and t.endswith(")"):
        t = t[1:-1].strip()

    # remove known type tags like "int", "bool", "boolean", "char" when prefixed
    t = re.sub(r"^(?:int|bool|boolean|char)\s+", "", t, flags=re.IGNORECASE)

    # strip quotes around simple tokens
...
def build(suite, compile, decompile, document, test, docker):
    ...
    if compile:
        ...
        run(
            ...
            + list(PurePosixPath(*a.relative_to(suite.workfolder).parts) for a in suite.sourcefiles()),
            logerr=log.warning,
            logout=log.info,
            timeout=600,
        )
        ...
        for cl in suite.classes():
            ...
            res, t = run(
                ...
                + [
                    "jvm2json",
                    "-s",
                    PurePosixPath(*suite.classfile(cl).relative_to(suite.workfolder).parts),
                ],
                logerr=log.warning,
            )
    ...
    if test:
        ...
        for case in suite.cases:
            ...
            log.info(f"input {case.input}")
            log.info(f"encoded {case.input.encode()}")

            folder = suite.classfiles_folder

            try:
                cp = str(PurePosixPath(*folder.relative_to(suite.workfolder).parts))
                # Clean and normalize input for the Java InputParser
                # Extract the raw string from Input(...) objects
                input_arg = case.input.encode()
